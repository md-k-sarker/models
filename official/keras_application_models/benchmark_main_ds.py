...
def my_input_fn():
  return dataset.generate_synthetic_input_dataset(
...
def run_keras_model_benchmark(_):
  ...
  # distribute_strategy=tf.contrib.distribute.CollectiveAllReduceStrategy(num_gpus_per_worker=0)
  # mirror strategy
  distribute_strategy=tf.contrib.distribute.MirroredStrategy(num_gpus_per_worker=0)
  ...
  max_steps = FLAGS.train_epochs * int(np.ceil(FLAGS.num_train_images / FLAGS.batch_size))
  print("max_steps: ", max_steps)
  keras_estimator = tf.keras.estimator.model_to_estimator(
                    keras_model=model, config=config, model_dir="/tmp/mdkamruz")

  tf.estimator.train_and_evaluate(
      keras_estimator,
      train_spec=tf.estimator.TrainSpec(input_fn=my_input_fn, max_steps=max_steps),
      eval_spec=tf.estimator.EvalSpec(input_fn=my_input_fn, steps=max_steps))
